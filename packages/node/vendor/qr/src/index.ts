...
const GF = {
  ...
  monomial(degree: number, coefficient: number) {
    ...
    const coefficients = fillArr(degree + 1, 0);
  ...
  addPoly(a: number[], b: number[]) {
    ...
    const sumDiff = fillArr(larger.length, 0);
    const lengthDiff = larger.length - smaller.length;
    const s = larger.slice(0, lengthDiff);
